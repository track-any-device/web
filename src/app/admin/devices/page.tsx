  ...
  return (
    <>
      ...
      <div className="tad-portal__body">
        <StatRow stats={[
          { label: 'Active', value: DEVICES.filter((d) => d.status === 'active').length },
          { label: 'Pending', value: DEVICES.filter((d) => d.status === 'pending').length },
          { label: 'Blocked', value: DEVICES.filter((d) => d.status === 'blocked').length },
        ]} />
        {/* SIM / GSM are admin-only per the privacy rules — never shown in tenant/my portals. */}
        <DataTable<Device>
          rows={DEVICES}
          columns={[
