...
const QUERY = gql`
  query Shop {
    deviceTypes { id name slug description price_usd price_pkr is_featured image }
  }
`;

interface DeviceType {
  id: string; name: string; slug: string; description: string | null;
  price_usd: number | null; price_pkr: number | null; is_featured: boolean;
  image: string | null;
}
...
export default async function ShopPage() {
  ...
  try {
    ...
    console.error('[shop] GraphQL query failed:', e);
  }
    ...
    <div className="max-w-7xl mx-auto px-6 py-14 space-y-16">
            ...
            {devices.map(d => (
              <a key={d.id} href={`/products/${d.slug}`}
                ...
                <div className="h-44 flex items-center justify-center text-5xl relative overflow-hidden"
                  ...
                  {d.image
                    ? <img src={d.image} alt={d.name} className="h-full w-full object-contain p-4" />
                    : '📡'}
                  {d.is_featured && (
      ...
      <section id="accessories">
        ...
        <div className="text-center py-16 rounded-2xl"
          ...
          <p style={{ color: '#475569' }}>Cables, chargers, and sensors — coming soon.</p>
        </div>
      </section>
