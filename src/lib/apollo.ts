...
const graphqlUrl = process.env.GRAPHQL_URL ?? 'https://graphql.track-any-device.com/graphql';
...
export function makeServerClient(): ApolloClient<unknown> {
  return new ApolloClient({
    ssrMode: true,
    link: new HttpLink({ uri: graphqlUrl, headers: serverHeaders(), fetch }),
    cache: new InMemoryCache(),
  } as ApolloClientOptions<unknown>);
}

export function makeAuthenticatedServerClient(token: string): ApolloClient<unknown> {
  return new ApolloClient({
    ssrMode: true,
    link: new HttpLink({
      uri: graphqlUrl,
      headers: { Authorization: `Bearer ${token}`, ...serverHeaders() },
      fetch,
    }),
    cache: new InMemoryCache(),
  } as ApolloClientOptions<unknown>);
